...
class LRU:
    ...
    def run_algorithm(self):
        ...
        while len(self.trace) > 0:
            ...
            if self.hit:
                ...
                csv_write.writerow(["Memory address: " , str(next_address[0]) , " VPN=", str(next_vpn) , ":: number " ,
                      str(self.PAGE_TABLE.total_memory_accesses) , "HIT"])
            elif not self.evict:
                  print( "Memory address: " + str(next_address[0]) + " VPN="+ str(next_vpn) + ":: number " + \
                      str(self.PAGE_TABLE.total_memory_accesses) + "\n\t->PAGE FAULT - NO EVICTION")
                  csv_write.writerow(["Memory address: ", str(next_address[0]), " VPN=", str(next_vpn), ":: number ",
                                      str(self.PAGE_TABLE.total_memory_accesses), "PAGE FAULT - NO EVICTION"])
                 ...
                 csv_write.writerow(["Memory address: ", str(next_address[0]), " VPN=", str(next_vpn), ":: number ",
                                     str(self.PAGE_TABLE.total_memory_accesses), "PAGE FAULT - EVICT CLEAN"])
                 # else, we have a page fault
                 self.PAGE_TABLE.page_faults += 1
            else:
                 print ("Memory address: " + str(next_address[0]) + " VPN="+ str(next_vpn) + ":: number " + \
                 ...
                 csv_write.writerow(["Memory address: ", str(next_address[0]), " VPN=", str(next_vpn), ":: number ",
                                     str(self.PAGE_TABLE.total_memory_accesses), "PAGE FAULT - EVICT DIRT"])
                 # else, we have a page fault
                 self.PAGE_TABLE.page_faults += 1
    ...
    def print_results(self):
        ...
        csv_write.writerow(["Number of frames:   ", str(len(self.PAGE_TABLE.frame_table))])
        csv_write.writerow([ "Total Memory Accesses: ", str(self.PAGE_TABLE.total_memory_accesses)])
        csv_write.writerow(["Total Page Faults: ", str(self.PAGE_TABLE.page_faults)])
        csv_write.writerow(["Total Writes to Disk: ", str(self.PAGE_TABLE.writes_to_disk)])
